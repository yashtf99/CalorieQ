"""
nutrition_llm_extract.py

Takes the raw OCR text of a nutrition label (from nutrition_ocr.classify_image)
and uses a Bedrock Nova model, via LangChain's structured-output support, to
extract the fields required by the meal creation API (POST /meals).

Dependencies:
    pip install langchain-aws pydantic boto3
    + AWS credentials configured (env vars, ~/.aws/credentials, or an
      assumed role) with access to the Bedrock model in region us-east-1.

Usage:
    from nutrition_ocr import classify_image
    from nutrition_llm_extract import extract_nutrition_from_text

    result = classify_image("label.jpg")
    if result["is_nutrition_label"]:
        nutrition = extract_nutrition_from_text(result["raw_text"])
"""
...
# --------------------------------------------------------------------------
# Output schema — mirrors the POST /meals fields. Range constraints from
# the spec are noted in the docstring/description only, per instruction
# to ignore them for extraction (an LLM enforcing a hard numeric range
# via schema validation would just fail/retry on any noisy OCR read,
# rather than returning what it saw).
# --------------------------------------------------------------------------
...
def extract_nutrition_from_text(
    raw_text: str,
    model: str = "global.amazon.nova-2-lite-v1:0",
    region_name: str = "us-east-1",
) -> dict:
    """Call the LLM with structured output and return a plain dict matching
    the POST /meals nutrition fields."""
...
# --------------------------------------------------------------------------
# Multimodal case — no nutrition label, just a photo of a food item
# (e.g. an apple, a plate of pav bhaji). The model has to identify the
# food, estimate the portion visible, and estimate nutrition for that
# portion. This is inherently a guess, not a read — the schema and
# prompt say so explicitly and always report it as such.
# --------------------------------------------------------------------------
...
def extract_nutrition_from_image(
    ...
    """Send the raw image to the multimodal LLM and get back an estimated
    food identification + nutrition breakdown for the visible portion."""
    import base64
    import mimetypes

    print("Extracting using Image")
...
if __name__ == "__main__":
    ...
    path = "./data/FoodItem-Images/food-item/eggs fried.jpg"
    classification = classify_image(path, debug_path="./data/FoodItem-Images/processed/debug_preprocessed.png")

    if classification["is_nutrition_label"]:
        nutrition = extract_nutrition_from_text(classification["raw_text"])
    else:
        nutrition = extract_nutrition_from_image(path)
